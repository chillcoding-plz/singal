...
def _load_single_file(
    ...
    for label, group in df.groupby('PRED_LABEL'):
...
def load_presegmented_files(
    ...
    # {radar_id: {window_id: WindowRecord}}
    radar_windows: dict[str, dict[int, WindowRecord]] = {}
    ...
    for file_idx, path in enumerate(sorted_paths):
        ...
        for label, group in df.groupby('PRED_LABEL'):
            ...
            w_start = file_idx * WINDOW_DURATION
            w_end = w_start + WINDOW_DURATION
            ...
            radar_windows.setdefault(rid, {})[file_idx] = rec
    ...
    for rid, wmap in radar_windows.items():
        ...
        sample_name = next(iter(wmap.values())).sample
        key = f"{sample_name}/{rid}"
